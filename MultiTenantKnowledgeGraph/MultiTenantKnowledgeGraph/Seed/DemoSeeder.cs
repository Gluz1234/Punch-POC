...
public static class DemoSeeder
{
    ...
    // ── Organizations ──────────────────────────────────────────────────
    private const string OrgMit       = "org-mit";
    private const string OrgDelft     = "org-delft";
    private const string OrgGoogle    = "org-google";
    private const string OrgMicrosoft = "org-microsoft";
    private const string OrgStartupX  = "org-startupx";
    private const string OrgCityAmst  = "org-amsterdam-city";

    // ── Locations ──────────────────────────────────────────────────────
    private const string LocAmsterdam = "loc-amsterdam";
    private const string LocDelft     = "loc-delft";
    private const string LocCambridge = "loc-cambridge-us";
    private const string LocLondon    = "loc-london";
    private const string LocBerlin    = "loc-berlin";

    // ── Skills ─────────────────────────────────────────────────────────
    private const string SkillJava       = "skill-java";
    private const string SkillCSharp     = "skill-csharp";
    private const string SkillPython     = "skill-python";
    private const string SkillML         = "skill-ml";
    private const string SkillKubernetes = "skill-kubernetes";
    private const string SkillReact      = "skill-react";
    private const string SkillDataEng    = "skill-data-engineering";
    private const string SkillCyber      = "skill-cybersecurity";
    private const string SkillUXDesign   = "skill-ux-design";

    // ── Education ──────────────────────────────────────────────────────
    private const string EduCsMit       = "edu-cs-mit";
    private const string EduDataScDelft = "edu-datascience-delft";
    private const string EduMbaOnline   = "edu-mba-online";
    private const string EduCyberCert   = "edu-cyber-cert";
    private const string EduUxCourse    = "edu-ux-course";
    private const string EduCloudCert   = "edu-cloud-cert";

    // ── Courses (new) ──────────────────────────────────────────────────
    private const string CourseAlgo       = "course-6006";
    private const string CourseML         = "course-6034";
    private const string CourseSystems    = "course-6033";
    private const string CourseDataSciDel = "course-cs4295";
    private const string CourseAI         = "course-cs4180";
    private const string CourseUXGoogle   = "course-ux-foundations";

    // ── Departments (new) ──────────────────────────────────────────────
    private const string DeptMITCS        = "dept-mit-cs";
    private const string DeptGoogleCloud  = "dept-google-cloud";
    private const string DeptMSAzure      = "dept-ms-azure";
    private const string DeptCityData     = "dept-city-data";

    // ── People ─────────────────────────────────────────────────────────
    private const string PersonSarah    = "person-sarah-chen";
    private const string PersonJames    = "person-james-okafor";
    private const string PersonMia      = "person-mia-rossi";
    private const string PersonLeon     = "person-leon-fischer";
    private const string PersonAnna     = "person-anna-kovacs";
    private const string PersonKarim    = "person-karim-benali";
    private const string PersonYuki     = "person-yuki-tanaka";
    private const string PersonElena    = "person-elena-popova";
    private const string PersonNoah     = "person-noah-patel";
    private const string PersonSofia    = "person-sofia-lima";
    private const string PersonMarcus   = "person-marcus-brown";
    private const string PersonIrina    = "person-irina-volkova";
    private const string PersonDavid    = "person-david-nguyen";
    private const string PersonAmelia   = "person-amelia-wright";
    private const string PersonOmar     = "person-omar-alfarsi";

    // Professor shared across MIT students
    private const string ProfessorId = "person-prof-johnson";
        ...
        PromotionService    promotionService)
    {
        ...
        await orgService.CreateOrUpdateAsync(new Organization { OrgId = OrgMit, Name = "MIT",
            OrganizationType = "University", Website = "https://mit.edu", Industry = "Education" });
        await orgService.CreateOrUpdateAsync(new Organization { OrgId = OrgDelft, Name = "TU Delft",
            OrganizationType = "University", Website = "https://tudelft.nl", Industry = "Education" });
        await orgService.CreateOrUpdateAsync(new Organization { OrgId = OrgGoogle, Name = "Google",
            OrganizationType = "Company", Industry = "Technology" });
        await orgService.CreateOrUpdateAsync(new Organization { OrgId = OrgMicrosoft, Name = "Microsoft",
            OrganizationType = "Company", Industry = "Technology" });
        await orgService.CreateOrUpdateAsync(new Organization { OrgId = OrgStartupX, Name = "StartupX",
            OrganizationType = "Company", Industry = "Fintech" });
        await orgService.CreateOrUpdateAsync(new Organization { OrgId = OrgCityAmst,
        ...
        await locationService.CreateOrUpdateAsync(new Location { LocationId = LocAmsterdam,
            Name = "Amsterdam", LocationType = "City", Latitude = 52.3676, Longitude = 4.9041, Population = 921402 });
        await locationService.CreateOrUpdateAsync(new Location { LocationId = LocDelft,
            Name = "Delft", LocationType = "City", Latitude = 52.0116, Longitude = 4.3571, Population = 103163 });
        await locationService.CreateOrUpdateAsync(new Location { LocationId = LocCambridge,
            Name = "Cambridge MA", LocationType = "City", Latitude = 42.3736, Longitude = -71.1097, Population = 118403 });
        await locationService.CreateOrUpdateAsync(new Location { LocationId = LocLondon,
            Name = "London", LocationType = "City", Latitude = 51.5074, Longitude = -0.1278, Population = 8982000 });
        await locationService.CreateOrUpdateAsync(new Location { LocationId = LocBerlin,
        ...
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillJava,       Name = "Java",             Category = "Programming Language" });
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillCSharp,     Name = "C#",               Category = "Programming Language" });
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillPython,     Name = "Python",           Category = "Programming Language" });
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillML,         Name = "Machine Learning", Category = "Data Science" });
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillKubernetes, Name = "Kubernetes",       Category = "DevOps" });
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillReact,      Name = "React",            Category = "Frontend" });
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillDataEng,    Name = "Data Engineering", Category = "Data Science" });
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillCyber,      Name = "Cybersecurity",    Category = "Security" });
        await skillService.CreateOrUpdateAsync(new Skill { SkillId = SkillUXDesign,   Name = "UX Design",        Category = "Design" });
        ...
        await educationService.CreateOrUpdateAsync(new Education { EducationId = EduCsMit,
        ...
        await educationService.CreateOrUpdateAsync(new Education { EducationId = EduDataScDelft,
        ...
        await educationService.CreateOrUpdateAsync(new Education { EducationId = EduMbaOnline,
            Title = "Online MBA", EducationType = "Degree", FieldOfStudy = "Business Administration" });
        await educationService.CreateOrUpdateAsync(new Education { EducationId = EduCyberCert,
            Title = "Certified Ethical Hacker (CEH)", EducationType = "Certificate", FieldOfStudy = "Cybersecurity" });
        await educationService.CreateOrUpdateAsync(new Education { EducationId = EduUxCourse,
            Title = "Google UX Design Certificate", EducationType = "Certificate", FieldOfStudy = "UX/UI Design" });
        await educationService.CreateOrUpdateAsync(new Education { EducationId = EduCloudCert,
        ...
        // MIT courses
        await promotionService.CreateOrUpdateCourseAsync(new Course { CourseId = CourseAlgo,
            OrgId = OrgMit, Name = "Introduction to Algorithms", Code = "6.006",
            Credits = 12, Level = "Undergraduate", AcademicTerm = "Fall 2023" });
        await promotionService.CreateOrUpdateCourseAsync(new Course { CourseId = CourseML,
            OrgId = OrgMit, Name = "Artificial Intelligence", Code = "6.034",
            Credits = 12, Level = "Undergraduate", AcademicTerm = "Fall 2023" });
        await promotionService.CreateOrUpdateCourseAsync(new Course { CourseId = CourseSystems,
            OrgId = OrgMit, Name = "Computer System Engineering", Code = "6.033",
            Credits = 12, Level = "Graduate", AcademicTerm = "Spring 2024" });

        // TU Delft courses
        await promotionService.CreateOrUpdateCourseAsync(new Course { CourseId = CourseDataSciDel,
            OrgId = OrgDelft, Name = "Machine Learning in Practice", Code = "CS4295",
            Credits = 5, Level = "Graduate", AcademicTerm = "Q1 2023" });
        await promotionService.CreateOrUpdateCourseAsync(new Course { CourseId = CourseAI,
            OrgId = OrgDelft, Name = "Deep Learning", Code = "CS4180",
            Credits = 5, Level = "Graduate", AcademicTerm = "Q2 2023" });

        // Google internal course
        await promotionService.CreateOrUpdateCourseAsync(new Course { CourseId = CourseUXGoogle,
        ...
        await promotionService.CreateOrUpdateDepartmentAsync(new Department { DepartmentId = DeptMITCS,
            OrgId = OrgMit, Name = "Dept. of Electrical Engineering & CS", Code = "EECS", HeadName = "Prof. Johnson" });
        await promotionService.CreateOrUpdateDepartmentAsync(new Department { DepartmentId = DeptGoogleCloud,
            OrgId = OrgGoogle, Name = "Google Cloud Engineering", Code = "GCE", HeadName = "Yuki Tanaka" });
        await promotionService.CreateOrUpdateDepartmentAsync(new Department { DepartmentId = DeptMSAzure,
            OrgId = OrgMicrosoft, Name = "Azure Platform Engineering", Code = "APE", HeadName = "Elena Popova" });
        await promotionService.CreateOrUpdateDepartmentAsync(new Department { DepartmentId = DeptCityData,
        ...
        await personService.CreateOrUpdateAsync(new Person { StrongId = ProfessorId,
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonSarah,
        ...
        await promotionService.PromoteToStudentAsync(new StudentProfile
            { StrongId = PersonSarah, StudentId = "MIT-2019-001", Gpa = 3.9,
              EnrollmentYear = 2019, EnrollmentStatus = "Graduated", StudyMode = "Full-time" });
        await promotionService.PromoteToResearcherAsync(new ResearcherProfile
            { StrongId = PersonSarah, OrcidId = "0000-0002-1111-2222",
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonSarah, EmployeeNumber = "G-44001",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonJames,
        ...
        await promotionService.PromoteToStudentAsync(new StudentProfile
            { StrongId = PersonJames, StudentId = "MIT-2020-014", Gpa = 3.7,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonJames, EmployeeNumber = "MS-88201",
        ...
        // Final labels: Person:Student:Researcher:Employee
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonMia,
        ...
        await promotionService.PromoteToStudentAsync(new StudentProfile
            { StrongId = PersonMia, StudentId = "DELFT-2021-033", Gpa = 4.0,
              EnrollmentYear = 2021, EnrollmentStatus = "Graduated", StudyMode = "Full-time" });
        await promotionService.PromoteToResearcherAsync(new ResearcherProfile
            { StrongId = PersonMia, OrcidId = "0000-0003-5555-6666",
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonMia, EmployeeNumber = "G-55789",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonLeon,
        ...
        await promotionService.PromoteToStudentAsync(new StudentProfile
            { StrongId = PersonLeon, StudentId = "DELFT-2022-077", Gpa = 3.5,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonLeon, EmployeeNumber = "AMS-DA-009",
        ...
        await promotionService.PromoteToResidentAsync(new ResidentProfile
            { StrongId = PersonLeon, ResidentId = "NL-AMS-001234",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonAnna,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonAnna, EmployeeNumber = "G-22341",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonKarim,
        ...
        await promotionService.PromoteToStudentAsync(new StudentProfile
            { StrongId = PersonKarim, StudentId = "MIT-2021-099", Gpa = 3.6,
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonYuki,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonYuki, EmployeeNumber = "G-11033",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonElena,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonElena, EmployeeNumber = "MS-10001",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonNoah,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonNoah, EmployeeNumber = "SX-007",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonSofia,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonSofia, EmployeeNumber = "AMS-DS-001",
        ...
        await promotionService.PromoteToResidentAsync(new ResidentProfile
            { StrongId = PersonSofia, ResidentId = "NL-AMS-005678",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonMarcus,
        ...
        await relService.AddEnrolledInAsync(new EnrolledInRelationship { PersonStrongId = PersonMarcus,
            OrgId = OrgDelft, TenantId = TenantDelft, Program = "MSc Data Science & AI", StartDate = new DateTime(2022,9,1) });
        await promotionService.PromoteToStudentAsync(new StudentProfile
            { StrongId = PersonMarcus, StudentId = "DELFT-2022-055", Gpa = 3.3,
              EnrollmentYear = 2022, EnrollmentStatus = "Active", StudyMode = "Part-time" });
        await promotionService.RegisterForCourseAsync(new RegisteredForRelationship
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonMarcus, EmployeeNumber = "AMS-INT-012",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonIrina,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonIrina, EmployeeNumber = "MS-33098",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonDavid,
        ...
        await promotionService.PromoteToStudentAsync(new StudentProfile
            { StrongId = PersonDavid, StudentId = "MIT-2022-044", Gpa = 3.4,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonDavid, EmployeeNumber = "SX-015",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonAmelia,
        ...
        await promotionService.PromoteToEmployeeAsync(new EmployeeProfile
            { StrongId = PersonAmelia, EmployeeNumber = "G-09922",
        ...
        // ─────────────────────────────────────────────────────────────
        await personService.CreateOrUpdateAsync(new Person { StrongId = PersonOmar,
